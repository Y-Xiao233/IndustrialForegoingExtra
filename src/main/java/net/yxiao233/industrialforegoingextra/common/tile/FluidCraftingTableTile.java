...
public class FluidCraftingTableTile extends IndustrialProcessingTile<FluidCraftingTableTile> {
    @Save
    public LockableInventoryBundle<FluidCraftingTableTile> inputs;
    @Save
    public SidedFluidTankComponent<FluidCraftingTableTile> inputFluid;
    @Save
    private SidedInventoryComponent<FluidCraftingTableTile> output;
    ...
    public FluidCraftingTableTile(BlockPos blockPos, BlockState blockState) {
        super(IFEBlocks.FLUID_CRAFTING_TABLE,102,41,blockPos,blockState);
        int slotSpacing = 22;

        this.addBundle(this.inputs = new LockableInventoryBundle<>(this,
                new SidedInventoryComponent<FluidCraftingTableTile>("input", 19 + slotSpacing, slotSpacing, 9, 0)
        ...
        this.addInventory(this.output = (SidedInventoryComponent<FluidCraftingTableTile>) new SidedInventoryComponent<FluidCraftingTableTile>("ot", 129, 18+slotSpacing, 1,2)
        ...
        this.addTank(this.inputFluid = (SidedFluidTankComponent<FluidCraftingTableTile>) new SidedFluidTankComponent<FluidCraftingTableTile>("input_fluid", FluidCraftingTableConfig.maxInputTankSize,17,19,1)
            ...
            public @NotNull List<IFactory<? extends IScreenAddon>> getScreenAddons() {
                return Collections.singletonList(() -> {
                    ...
                    return new StateButtonAddon(this, buttonInfo) {
                        public int getState() {
                            return FluidCraftingTableTile.this.autoCraft ? 0 : 1;
                        }
                    };
                });
            }
        ...
        this.addButton(this.buttonComponent2 = (new ButtonComponent(120, 84, 14, 14) {
            ...
            public @NotNull List<IFactory<? extends IScreenAddon>> getScreenAddons() {
                return Collections.singletonList(() -> {
                    ...
                    return new StateButtonAddon(this, buttonInfo) {
                        public int getState() {
                            return FluidCraftingTableTile.this.isFluidRender ? 0 : 1;
                        }
                    };
                });
            }
    ...
    private void checkForRecipe(){
        if(isServer()){
            if(shapelessRecipe != null && shapelessRecipe.matches(inputs.getInventory(),inputFluid)){
                return;
            }

            if(shapedRecipe != null && shapedRecipe.matches(inputs.getInventory(),inputFluid)){
                return;
            }

            shapelessRecipe = RecipeUtil.getRecipes(this.level, IFERecipes.SHAPELESS.asType()).stream().filter(recipe -> recipe.matches(inputs.getInventory(),inputFluid)).findFirst().orElse(null);

            if(shapelessRecipe == null){
                shapedRecipe = RecipeUtil.getRecipes(this.level, IFERecipes.SHAPED.asType()).stream().filter(recipe -> recipe.matches(inputs.getInventory(),inputFluid)).findFirst().orElse(null);
            }
        }
    }
    ...
    @Override
    public void serverTick(@NotNull Level level, @NotNull BlockPos pos, @NotNull BlockState state, @NotNull FluidCraftingTableTile blockEntity) {
        ...
        if(!this.autoCraft){
            ...
            if(lastTimeShapedRecipe != null && lastTimeShapelessRecipe == null){
                if(shapedOnFinish && !lastTimeShapedRecipe.matches(inputs.getInventory(),inputFluid)) {
            ...
            if(lastTimeShapelessRecipe != null && lastTimeShapedRecipe == null){
                if(shapelessOnFinish && !lastTimeShapelessRecipe.matches(inputs.getInventory(),inputFluid)) {
                    this.output.setStackInSlot(0, ItemStack.EMPTY);
    ...
    private boolean isInputSlotsAllMatchEmpty(){
        ...
        for (int i = 0; i < this.inputs.getInventory().getSlots(); i++) {
            if(!this.inputs.getInventory().getStackInSlot(i).isEmpty()){
                return false;
            }else{
                allMatch = true;
            }
    ...
    @NotNull
    @Override
    public FluidCraftingTableTile getSelf() {
        return this;
    }

    @NotNull
    @Override
    protected EnergyStorageComponent<FluidCraftingTableTile> createEnergyStorage() {
    ...
    public void saveSettings(Player player, CompoundTag tag) {
        ...
        tag.putBoolean("FTC_locked", this.inputs.isLocked());
        CompoundTag filterTag = new CompoundTag();

        for(int i = 0; i < this.inputs.getFilter().length; ++i) {
            filterTag.put("" + i, this.inputs.getFilter()[i].saveOptional(this.level.registryAccess()));
        }

        tag.put("FTC_filter", filterTag);
    }
    ...
    public void loadSettings(Player player, CompoundTag tag) {
        ...
        if (tag.contains("FCT_locked")) {
            this.inputs.setLocked(tag.getBoolean("FCT_locked"));
        }

        String psFilter;
        if (tag.contains("FTC_filter")) {
            for(Iterator<String> var3 = tag.getCompound("FTC_filter").getAllKeys().iterator(); var3.hasNext(); this.inputs.getFilter()[Integer.parseInt(psFilter)] = ItemStack.parseOptional(this.level.registryAccess(), tag.getCompound("FTC_filter").getCompound(psFilter))) {
                psFilter = var3.next();
            }
        }
    }

...
public abstract class IFEItemFuelGeneratorTile<T extends IFEItemFuelGeneratorTile<T>> extends IFEIndustrialGeneratorTile<T> {
    ...
    public IFEItemFuelGeneratorTile(BlockWithTile blockWithTile, BlockPos blockPos, BlockState blockState) {
        super(blockWithTile, blockPos, blockState);
        init();
    }

    public abstract SlotElements getSlotElements();
    public abstract Class<?> getGeneratorConfigClass();
    public abstract ItemStack getConsumeFuel();

    public void init(){
        SlotElements elements = getSlotElements();
        this.addInventory(this.input = (SidedInventoryComponent<T>) new SidedInventoryComponent<T>(elements.getId(), elements.getXPos(), elements.getYPos(), elements.getSize(), elements.getPosition())
                .setColor(elements.getColor())
                .setRange(elements.getRange()[0],elements.getRange()[1])
                .setOutputFilter(elements.getOutputFilter())
                .setInputFilter(elements.getInputFilter())

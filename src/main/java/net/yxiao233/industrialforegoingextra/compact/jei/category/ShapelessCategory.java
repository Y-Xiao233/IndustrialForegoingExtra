...
@JeiCategory
public class ShapelessCategory extends AbstractJEICategory<RecipeHolder<ShapelessRecipe>> {
    public static final Component TITLE = Component.translatable("jei.industrialforegoingextra.fluid_shapeless");
    private final IDrawable bigTank_input1;
    public ShapelessCategory(IGuiHelper helper) {
        super(helper, IFERecipeType.SHAPELESS, TITLE, IFEBlocks.FLUID_CRAFTING_TABLE.asItem(),160, 82);
    ...
    public <I extends RecipeInput, R extends Recipe<I>> RecipeType<R> getRecipe() {
        return (RecipeType<R>) IFERecipes.SHAPELESS.asType();
    }

    @Override
    public void setRecipe(@NotNull IRecipeLayoutBuilder builder, @NotNull RecipeHolder<ShapelessRecipe> recipe, @NotNull IFocusGroup iFocusGroup) {
        //Input
        int x = 31;
        int y = 15;
        for (int i = 0; i < 9; i++) {
    ...
    @Override
    public void draw(@NotNull RecipeHolder<ShapelessRecipe> recipe, @NotNull IRecipeSlotsView recipeSlotsView, @NotNull GuiGraphics guiGraphics, double mouseX, double mouseY) {
        //Input
        int x = 31;
        int y = 15;
        for (int i = 0; i < 9; i++) {
        ...
        //Shapeless icon
        drawTextureWithTooltip(guiGraphics, AllGuiTextures.SHAPELESS,Component.translatable("jei.industrialforegoingextra.shapeless"),119,17,mouseX,mouseY);
    }
}
